...
bool  COxRobot::OnEventTimer(UINT nTimerID)
{
	...
	case IDI_CHECK_BANKER:		//检查上庄
	{
									...
									if (ApplyListLength()<COxRobot::m_byAppCountLimit&&!g_FishServer.GetNiuNiuControl().GetNiuNiuManager()->HaveApply(UserId()))//BankerId() == 0 ||
									{
										//空庄
										m_nWaitBanker++;
										...
										if (m_bBnkerRobot&& m_nWaitBanker > m_nRobotWaitBanker)
										{
											...
											if (UserGold() > GameConfig().JoinBankerGlobelSum)
											{
												...
												m_nWaitBanker = 0;
												g_FishServer.GetNiuNiuControl().GetNiuNiuManager()->OnRoleJoinWriteBankerList(UserId());
											}
										}
									}
									//else if (BankerId() != 0)
	...
	{									
									g_FishServer.GetNiuNiuControl().GetNiuNiuManager()->OnRoleCanelBanker(UserId());
									return false;
	}
	...
	default:
	{
			   ...
			   {				  				 
				   ...
				   g_FishServer.GetNiuNiuControl().GetNiuNiuManager()->OnRoleBetGlobel(UserId(), nChipArea, nJetton);
...
{
	return g_FishServer.GetNiuNiuControl().GetNiuNiuManager()->BankerId();
}

DWORD COxRobot::BankerGold()
{
	return g_FishServer.GetNiuNiuControl().GetNiuNiuManager()->BankerGold();
}

WORD  COxRobot::ApplyListLength()
{
	return g_FishServer.GetNiuNiuControl().GetNiuNiuManager()->ApplyListLength();
}
...
void COxRobotManager::Update()
{
	...
	if (m_allrobot.size() < COxRobot::m_byRobotCount)
	{
		...
		{	
			...
			g_FishServer.GetNiuNiuControl().OnRoleJoin(pInfo->dwUserID);
			//g_FishServer.GetNiuNiuControl().GetNiuNiuManager()->OnRoleJoinNiuNiuTable(pInfo->dwUserID);
	...
	{		
		if (!(*it)->Update() && g_FishServer.GetNiuNiuControl().GetNiuNiuManager()->IsGameEnd())
		{		
			g_FishServer.GetRobotManager().OnNonUseRobot((*it)->UserId());
			g_FishServer.GetNiuNiuControl().OnRoleLeave((*it)->UserId());
			//g_FishServer.GetNiuNiuControl().GetNiuNiuManager()->OnRoleLeaveNiuNiuTable((*it)->UserId());
			delete (*it);
