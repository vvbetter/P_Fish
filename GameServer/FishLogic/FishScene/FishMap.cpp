...
bool LoadRandFlow(XMLElement *pFlow, FishMapData *pFishMapData, wstring &file)
{
	...
	while (pEle)
	{
		...
		{
			Log(L"配置文件未包含FLow属性:%s", file.c_str());
			return false;
		}
...
bool LoadFixedFlow(XMLElement *pFixedFlow, FishMapData *pFishMapData, wstring &file)
{
	...
	{
		Log(L"配置文件缺少固定或顺序节点:%s", file.c_str());
		return false;
	}
...
bool LoadPackage(XMLElement *pPackage, FishMapData *pFishMapData, wstring &file)
{
	...
	while (pEle)
	{
		...
		if (pcAtt == NULL)
		{
			Log(L"配置文件未包含StartSeconds属性:%s", file.c_str());
			return false;
		}
		...
		if (pcAtt == NULL)
		{
			Log(L"配置文件未包含EndSeconds属性:%s", file.c_str());
			return false;
		}
		...
		if (pcAtt == NULL)
		{
			Log(L"配置文件未包含IntervalSeconds属性:%s", file.c_str());
			return false;
		}
		...
		if (pcAtt == NULL)
		{
			Log(L"配置文件未包含Chance属性:%s", file.c_str());
			return false;
		}
		...
		if (pcAtt == NULL)
		{
			Log(L"配置文件未包含MaxCount属性:%s", file.c_str());
			return false;
		}
		...
		if (pcAtt == NULL)
		{
			Log(L"配置文件未包含PackageType属性:%s", file.c_str());
			return false;
		}
...
bool FishMap::LoadAllMap(const WCHAR *pcDir)
{
	...
	for (UINT i = 0; i < fileList.size(); ++i)
	{
		...
		if (pcAtt == NULL)
		{
			Log(L"配置文件未包含BackgroundImage属性:%s", file.c_str());
			return false;
		}
		...
		if (pcAtt == NULL)
		{
			Log(L"配置文件未包含Repeat属性:%s", file.c_str());
			return false;
		}
		...
		if (pcAtt == NULL)
		{
			Log(L"配置文件未包含Name属性:%s", file.c_str());
			return false;
		}
		...
		{
			Log(L"配置文件不正确，缺少固定、随机或者红包节点:%s", file.c_str());
			return false;
		}
		
		if (pFishMapData->FLowList.size() == 0)
		{
			Log(L"配置文件流程数量为0:%s", file.c_str());
			return false;
		}
		...
		for (UINT j = 0; j < pFishMapData->FlowOrderData.FlowIndex.size(); ++j)
		{
			...
			if (pFlow == NULL)
			{
				Log(L"地图中的固定流程为NUMM:%s ：%d", file.c_str(), j);
				return false;
			}
		}
		for (UINT j = 0; j < pFishMapData->FlowOrderData.OrderIndex.size(); ++j)
		{
			...
			{
				Log(L"地图中的固定流程顺序不正确:%s ：%d", file.c_str(), idx);
				return false;
			}
		}
		...
		{
			Log(L"地图中的固定流程为0，但有顺序设置:%s ：%d", file.c_str(), pFishMapData->FlowOrderData.OrderIndex.size());
			return false;
		}
		...
		for (UINT j = 0; j < pFishMapData->FLowList.size(); ++j)
		{
			...
			if (pFlow == NULL)
			{
				Log(L"地图中的流程为NUMM:%s ：%d", file.c_str(), j);
				return false;
			}
	...
	Log(L"加载地图:%d", ms_AllMapList.size());
	return true;
}
