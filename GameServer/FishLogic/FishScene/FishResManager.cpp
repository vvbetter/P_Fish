...
bool FishResManager::InitGroup(const WCHAR *pcDir)
{
	...
	while (1)
	{
		...
		if (pd.FishGroupByPathGroup)
		{
			...
			{
				Log(L"鱼群中不存在的鱼索引:%d : %d", idx - 1, pd.FishIndex);
				return false;
			}
			if (GetPathGroup(pd.PathGroupIndex) == NULL)
			{
				Log(L"鱼群中不存在的路径群索引:%d : %d", idx - 1, pd.PathGroupIndex);
				return false;
			}
		}
		else
		{
			...
			for (int i = 0; i < pd.PathCount; ++i)
			{
				...
				{
					Log(L"2鱼群中不存在的路径索引:%d : %d", idx - 1, pd.pPathList[i]);
					return false;
				}
			}
			...
			for (int i = 0; i < pd.GroupDataCount; ++i)
			{
				...
				{
					Log(L"2鱼群中不存在的鱼索引:%d : %d", idx - 1, gd.FishIndex);
					return false;
				}
	...
	Log(L"加载鱼群数:%d ", m_GroupList.size());
	return true;
}
...
bool FishResManager::InitFlow(const WCHAR *pcDir)
{
	...
	while (1)
	{
		...
		for (int i = 0; i < fd.FlowTimeCount; ++i)
		{
			...
			{
				Log(L"流程中不存在的鱼群ID:%d, %d", idx - 1, fd.pFlowTimeList[i].GroupID);
				return false;
			}
			//Log("GroupID:%d,startTime:%f,endTime:%f", fd.pFlowTimeList[i].GroupID, fd.pFlowTimeList[i].StartTime, fd.pFlowTimeList[i].EndTime);
		}
		...
		for (int i = 0; i < fd.GroupCount; ++i)
		{
			...
			{
				Log(L"2流程中不存在的鱼群ID:%d, %d", idx - 1, fd.pFlowGroupList[i].GroupID);
				return false;
			}
		}
		UINT magic;
...
bool FishResManager::InitPathGroup(const WCHAR *pcDir)
{
	...
	Log(L"加载路径群数:%d ", m_PathGroupList.size());
	return true;
}
...
bool FishResManager::InitFishData(const WCHAR *pcDir)
{
	...
	while (1)
	{
		...
		if (magic != FILE_END_MAGIC)
		{
			Log(L"FishData文件结尾不正确!");
		}
		fclose(pFile);
	}
	Log(L"加载鱼信息数:%d ", m_FishDataList.size());
	return true;
}
