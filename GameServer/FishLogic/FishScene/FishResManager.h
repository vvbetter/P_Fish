...
public:
	...
	GroupData* GetGroup(UINT idx)
	{
		...
		{
			Log(L"鱼群索引不正确:%d, %d", idx, m_GroupList.size());
			return NULL;
		}
		else
	...
	FlowData* GetFlow(UINT idx)
	{
		...
		{
			Log(L"流程索引不正确:%d, %d", idx, m_FlowList.size());
			return NULL;
		}
		else
	...
	PathData* GetPath(UINT idx)
	{
		...
		{
			Log(L"路径索引不正确:%d, %d", idx, m_PathList.size());
			return NULL;
		}
		else
	...
	PathData* GetPathInv(UINT idx)
	{
		...
		{
			Log(L"路径索引不正确:%d, %d", idx, m_PathInvList.size());
			return NULL;
		}
		else
	...
	FishData* GetFishData(UINT idx)
	{
		...
		{
			Log(L"鱼数据索引不正确:%d, %d", idx, m_FishDataList.size());
			return NULL;
		}
		else
	...
	PathGroupData* GetPathGroup(UINT idx)
	{
		...
		{
			Log(L"路径群索引不正确:%d, %d", idx, m_PathGroupList.size());
			return NULL;
		}
		else
	...
	PathGroupData* GetPathGroupInv(UINT idx)
	{
		...
		{
			Log(L"路径群索引不正确:%d, %d", idx, m_PathGroupInvList.size());
			return NULL;
		}
		else
