...
enum NetCmdType
{
	...
	CMD_SKILL_DOUBLE_FIRE = Main_Game << 8 | SubMainCmdType::SUB_SKILL_DOUBLE_FIRE,
	CMD_SKILL_TIMESTOP = Main_Game << 8 | SubMainCmdType::SUB_SKILL_TIMESTOP,
...
struct NetCmdClientShowOthers :public NetCmd
{
	int64 playerid;
};
struct NetCmdOtherInfo :public NetCmd
{
	int64   playerId;//查看的玩家ID
	INT   vipLevel;//vip等级
	char   icon[ICON_LENGTH];//icon
	char   name[MAX_NICKNAME + 1];//显示姓名
	INT    sex;//性别
	INT   titleId;//成就ID
	double   gameWinMaxMoney;//单局最高盈利
	double   money;//金额
	INT   charmValue;//魅力值
