...
bool FishConfig::LoadConfigFilePath()
{
	...
	if (!LoadFishZPConfig(pFishConfig))
	{
		ASSERT(false);
		return false;
	}
...
bool FishConfig::LoadFishMiniGameConfig(WHXmlNode* pFishConfig)
{
	...
	if (!pFishCar->GetAttribute(TEXT("VipNonBetSum"), m_MiniGameConfig.carConfig.VipNonBetSum))
		return false;
	if (!pFishCar->GetAttribute(TEXT("MaxMultiple"), m_MiniGameConfig.carConfig.MaxMultiple))
		return false;
	//Types
	...
	//4.通比牛牛
	WHXmlNode* pFishFiveNiuNiu = pFishMiniGame->GetChildNodeByName(TEXT("fiveniuniu"));
	if (!pFishFiveNiuNiu)
	{
		ASSERT(false);
		return false;
	}
	WHXmlNode* pFishRates = pFishFiveNiuNiu->GetChildNodeByName(TEXT("Rates"));
	if (!pFishRates)
	{
		ASSERT(false);
		return false;
	}
	WHXmlNode* pFishRate = pFishRates->GetChildNodeByName(TEXT("Rate"));
	while (pFishRate)
	{
		tagFiveNiuNiuRate pRate;
		pRate.MaxRate = 0;
		if (!pFishRate->GetAttribute(TEXT("ID"), pRate.RateTypeID))
		{
			ASSERT(false);
			return false;
		}
		WHXmlNode* pFishLevel = pFishRate->GetChildNodeByName(TEXT("Level"));
		while (pFishLevel)
		{
			BYTE ID = 0;
			if (!pFishLevel->GetAttribute(TEXT("ID"), ID))
			{
				ASSERT(false);
				return false;
			}
			BYTE Rate = 0;
			if (!pFishLevel->GetAttribute(TEXT("Rate"), Rate))
			{
				ASSERT(false);
				return false;
			}
			pRate.NiuNiuLevelRate.insert(HashMap<BYTE, BYTE>::value_type(ID,Rate));
			if (pRate.MaxRate < Rate)
				pRate.MaxRate = Rate;

			pFishLevel = pFishLevel->GetNextSignelNode();
		}
		m_MiniGameConfig.fiveniuniuConfig.NiuNiuRateMap.insert(HashMap<BYTE, tagFiveNiuNiuRate>::value_type(pRate.RateTypeID,pRate));
		pFishRate = pFishRate->GetNextSignelNode();
	}

	WHXmlNode* pFishRooms = pFishFiveNiuNiu->GetChildNodeByName(TEXT("Rooms"));
	if (!pFishRooms)
	{
		ASSERT(false);
		return false;
	}
	WHXmlNode* pFishRoom = pFishRooms->GetChildNodeByName(TEXT("Room"));
	while (pFishRoom)
	{
		tagNiuNiuRoomConfig pRoom;

		if (!pFishRoom->GetAttribute(TEXT("ID"), pRoom.bRoomID))
		{
			ASSERT(false);
			return false;
		}
		if (!pFishRoom->GetAttribute(TEXT("TypeID"), pRoom.bRoomType))
		{
			ASSERT(false);
			return false;
		}
		if (!pFishRoom->GetAttribute(TEXT("JoinGlobelSum"), pRoom.dwJoinRoomGlobel))
		{
			ASSERT(false);
			return false;
		}
		if (!pFishRoom->GetAttribute(TEXT("InitTableSum"), pRoom.wInitTableSum))
		{
			ASSERT(false);
			return false;
		}
		if (!pFishRoom->GetAttribute(TEXT("BeExitTableGlobel"), pRoom.dwTableMinGlobel))
		{
			ASSERT(false);
			return false;
		}
		if (!pFishRoom->GetAttribute(TEXT("BetGlobelSum"), pRoom.dwOnceBetGlobel))
		{
			ASSERT(false);
			return false;
		}
		if (!pFishRoom->GetAttribute(TEXT("BeginOpenRoleSum"), pRoom.bOpenGameMinRoleSum))
		{
			ASSERT(false);
			return false;
		}
		if (!pFishRoom->GetAttribute(TEXT("MaxTableSum"), pRoom.bTableMaxRoleSum))
		{
			ASSERT(false);
			return false;
		}
		if (!pFishRoom->GetAttribute(TEXT("RateTypeID"), pRoom.bRateID))
		{
			ASSERT(false);
			return false;
		}
		if (m_MiniGameConfig.fiveniuniuConfig.NiuNiuRateMap.count(pRoom.bRateID) != 1)
		{
			ASSERT(false);
			return false;
		}
		if (!pFishRoom->GetAttribute(TEXT("BeginWriteSec"), pRoom.wBeginWriteSec))
		{
			ASSERT(false);
			return false;
		}
		if (!pFishRoom->GetAttribute(TEXT("EndWriteSec"), pRoom.wEndWriteSec))
		{
			ASSERT(false);
			return false;
		}
		m_MiniGameConfig.fiveniuniuConfig.NiuNiuRoomMap.insert(HashMap<BYTE, tagNiuNiuRoomConfig>::value_type(pRoom.bRoomID, pRoom));

		pFishRoom = pFishRoom->GetNextSignelNode();
	}

	return true;
}
bool FishConfig::LoadFishZPConfig(WHXmlNode* pFishConfig)
{
	if (!pFishConfig)
	{
		ASSERT(false);
	...
	WHXmlNode* pFishZP = pFishConfig->GetChildNodeByName(TEXT("FishZP"));
	if (!pFishZP)
	{
		ASSERT(false);
		return false;
	}
	if (!pFishZP->GetAttribute(TEXT("FreeNum"), m_ZPConfig.freeNum))
	{
		ASSERT(false);
		return false;
	}
	if (!pFishZP->GetAttribute(TEXT("VipFreeNum"), m_ZPConfig.vipFreeNum))
	{
		ASSERT(false);
		return false;
	}
	if (!pFishZP->GetAttribute(TEXT("OpenUseGold"), m_ZPConfig.useGoldNum))
	{
		ASSERT(false);
		return false;
	}
	m_ZPConfig.zpItemVec.clear();
	WHXmlNode* pItem = pFishZP->GetChildNodeByName(TEXT("ZPItem"));
	DWORD TotalRate = 0;
	while (pItem)
	{
		tagZPItemInfo pInfo;
		if (!pItem->GetAttribute(TEXT("RewardID"), pInfo.wRewardID))
			return false;
		if (!pItem->GetAttribute(TEXT("Rate"), pInfo.dwRateValue))
			return false;
		TotalRate += pInfo.dwRateValue;
		pInfo.dwRateValue = TotalRate;
		m_ZPConfig.zpItemVec.push_back(pInfo);
		pItem = pItem->GetNextSignelNode();
	}
