...
bool FishServerConfig::LoadDBConfig(WHXmlNode* pFishConfig)
{
	...
	while (pDBServer)
	{
		...
		WHXmlNode* pSqlConnection = pDBServer->GetChildNodeByName(TEXT("SqlConnection"));
		while (pSqlConnection)
		{
			...
			if (!pSqlConnection->GetAttribute(TEXT("DBName"), TempStr, MySQLLength))
				return false;

			pChar = WCharToChar(TempStr, TempCount);
			strncpy_s(info.MySQlDBName, TempCount + 1, pChar, TempCount);
			free(pChar);


			if (!pSqlConnection->GetAttribute(TEXT("DBIP"), TempStr, MySQLLength))
				return false;
			pChar = WCharToChar(TempStr, TempCount);
			strncpy_s(info.MySQlIP, TempCount + 1, pChar, TempCount);
			free(pChar);


			if (!pSqlConnection->GetAttribute(TEXT("DBUserName"), TempStr, MySQLLength))
				return false;
			pChar = WCharToChar(TempStr, TempCount);
			strncpy_s(info.MySQlUserName, TempCount + 1, pChar, TempCount);
			free(pChar);

			if (!pSqlConnection->GetAttribute(TEXT("DBUserPassword"), TempStr, MySQLLength))
				return false;
			pChar = WCharToChar(TempStr, TempCount);
			strncpy_s(info.MySQlUserPassword, TempCount + 1, pChar, TempCount);
			free(pChar);

			pDBConfig.DBConnectionVec.push_back(info);

			pSqlConnection = pSqlConnection->GetNextSignelNode();
		}
...
bool FishServerConfig::LoadGameServerConfig(WHXmlNode* pFishConfig)
{
	...
	while (pGameServer)
	{
		...
		if (!pGameServer->GetAttribute(TEXT("BeginRobotUserID"), pGameConfig.BeginRobotUserID))
			return false;
		if (!pGameServer->GetAttribute(TEXT("TestRobotNumber"), pGameConfig.TestRobotNumber))
			return false;
...
bool FishServerConfig::LoadOperateConfig(WHXmlNode* pFishConfig)
{
	...
	{
		TCHAR strHypayKey[FileXmlStrLength] = { 0 };
		if (!pOperateServer->GetAttribute(TEXT("HYKey"), strHypayKey, CountArray(strHypayKey)))
			return false;
		UINT Count = 0;
		char* pChar = WCharToChar(strHypayKey, Count);
		strncpy_s(m_OperateConfig.HYKey, Count + 1, pChar, Count);
		free(pChar);
	}

	if (!pOperateServer->GetAttribute(TEXT("HYAgentID"), m_OperateConfig.HYAgentID))
		return false;
