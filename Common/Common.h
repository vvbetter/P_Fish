...
extern const string GetTimeStr();
extern const wstring GetTimeStrW();
//全局的参数设置
enum
{
...
inline bool CheckChangeDWORDValue(int64 nSrcValue, int64 ChangeValue)
{
	...
	{
		return static_cast<int64>(ChangeValue*-1) <= nSrcValue;
	}
}
...
template<typename T>
//inline 
void SqlitMsg(T* OldMsg, DWORD OldMsgSize, DWORD ArraySum, bool IsSendToClient, std::vector<T*>& pVec)
{
	//分包函数
	pVec.clear();
...
static void SplitString2Vector(const std::string& s, std::vector<std::string>& v, const std::string& c)
{
...
{
public:
	explicit ITimer(){ m_TimerEventVec.clear(); }
	virtual ~ITimer(){ m_TimerEventVec.clear(); }

	ITimer(const ITimer&) = delete;
	ITimer& operator=(const ITimer&) = delete;

	typedef void(T::*TimerFun)(DWORD); //定义的函数指针

	struct tagTimerEvent
	{
		DWORD		dwWriteMilliSecond;//等待的毫秒数
		TimerFun	pfFun;//回调函数指针
		DWORD		dwStepNum;//循环次数
		DWORD		dwLogTime;//上次更新记录时间
	};

	virtual void AddTimer(DWORD	dwWriteMilliSecond, TimerFun pfFun,bool IsFirst = false, DWORD dwStepNum = 0xffffffff) final
	{
		tagTimerEvent tEvent;
		tEvent.dwStepNum = dwStepNum;
		tEvent.dwWriteMilliSecond = dwWriteMilliSecond;
		tEvent.pfFun = pfFun;
		tEvent.dwLogTime = IsFirst?0:timeGetTime();
		m_TimerEventVec.push_back(tEvent);
	}

	virtual void Clear() final
	{
		m_TimerEventVec.clear();
	}

	virtual void OnUpdateTimer(DWORD dwTimer = timeGetTime()) final
	{
		if (m_TimerEventVec.empty())
			return;
		T* pT = dynamic_cast<T*>(this);
		if (nullptr == pT)
			return;
		std::vector<tagTimerEvent>::iterator Iter = m_TimerEventVec.begin();
		for (; Iter != m_TimerEventVec.end();)
		{
			if (dwTimer >= Iter->dwLogTime + Iter->dwWriteMilliSecond)
			{
				//触发更新
				(pT->*Iter->pfFun)(dwTimer);
				Iter->dwLogTime = dwTimer;
				Iter->dwStepNum -= (0xffffffff == Iter->dwStepNum ? 0 : 1);
				if (0 == Iter->dwStepNum)
				{
					Iter = m_TimerEventVec.erase(Iter);
					continue;
				}
			}
			++Iter;
