...
bool HttpServer::SplitStr(HttpClientData *pc, UINT endSize, HttpRecvData &recvData)
{
	...
	if (validFields == pAddrData->ValidationFields.size())
	{
		if (pAddrData->pCallback)
		{
			recvData.IsCallbackSend = true;
			recvData.ResponseStr = pAddrData->pCallback->RequestCallback(&recvData);
		}
		return true;
	}
	else
			...
			else if (nSize > 0)
			{
				...
				if (endSize != END_ERR)
				{
					...
					if (pc->IsPost)
					{
						...
						{
							if (rd.IsCallbackSend)
							{
								SendResponse(pc, strTime, rd.ResponseStr.c_str());
							}
							else
							{
