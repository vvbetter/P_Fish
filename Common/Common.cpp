...
bool CreateSocket(UINT type, USHORT port, uint recvBuffSize, uint sendBuffSize, CreateSocketData &csd, UINT localIP)
{
	...
	if (type & CST_BIND)
	{
		...
		if (ret == SOCKET_ERROR)
		{
			closesocket(s);
			char buf[30] = { 0 };
			GetIPString(addr.sin_addr.S_un.S_addr, buf);
			Log("Bind Failed:error code=%d,ip = %s,port=%d", WSAGetLastError(), buf, ntohs(addr.sin_port));
			return false;
		}
		...
		//Log("retrun ip = %d,port=%d", addr.sin_addr.S_un.S_addr , ntohs(addr.sin_port));
	}
