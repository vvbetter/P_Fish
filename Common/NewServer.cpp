...
bool NewServer::Init(const ServerInitData &data, bool bTCP)
{
	...
	//PostAccept(pio);
	Log("%d号服务已启动(ListenPort:%d, SendThreadNum:%d, RecvThreadNum:%d, SendBuff:%d, RecvBuff:%d, SendCmdCount:%d, RecvCmdCount:%d, BuffSize:%d, SceneTick:%d, Timeout:%d, Valid:%d)",
		m_InitData.ServerID,
		m_InitData.Port,
...
void NewServer::_ThreadAccept()
{
	...
	while (m_bRun)
	{
		...
		for (UINT i = 0; i < clientList.size(); ++i)
		{
			...
			{
				Log("Accecpt 客户端 recv ERROR: ret=%d ,error code %d",ret, WSAGetLastError());
				closesocket(acd.Socket);
				acd.Socket = NULL;
				continue;
			}
