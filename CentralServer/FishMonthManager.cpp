	...
	if (m_LastMonthInfo.OnAddGameServerFinish())
	{
		...
		for (int i = 0; Iter != m_LastMonthInfo.Lastm_RoleVec.end(); ++Iter)
		{
			...
			if (i <= 2)
			{
				//前三名发送提示公告
				TCHAR tempMsg[128] = { 0 };
				_sntprintf_s(tempMsg, CountArray(tempMsg), TEXT("恭喜%s获得比赛的第%u名"), (*Iter)->NickName, i + 1);
			...
			_sntprintf_s(MailInfo.Context, CountArray(MailInfo.Context), TEXT("恭喜您获得{MonthName:MonthID=%d}倍金币赛 的第%d名 得到以下奖励"), m_MonthID, i + 1);
