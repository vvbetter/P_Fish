...
void ClientListManager::RsgUser(DWORD UserID, BYTE GameID)
{
	...
	//Log("玩家 ID: %d  进入中央服务器 绑定GameServerID为: %d",UserID,GameID);
}
...
void ClientListManager::RsgGameHall(BYTE HallID)
{
	...
	Log("游戏大厅进入游戏 IP:%d.%d.%d.%d", pClient->IP & 0xff, (pClient->IP >> 8) & 0xff, (pClient->IP >> 16) & 0xff, (pClient->IP >> 24) & 0xff);
}
void ClientListManager::UnRsgGameHall(BYTE HallID)
{
	...
	Log("游戏大厅离开 IP:%d.%d.%d.%d", pClient->IP & 0xff, (pClient->IP >> 8) & 0xff, (pClient->IP >> 16) & 0xff, (pClient->IP >> 24) & 0xff);
}
